...
import type { ReactivateDeviceResponseDTO } from "../dto/reactivate-device.response.dto.js"
import { SectorNotFoundError, OltNotFoundError } from "../../../domain/olt/exceptions/index.js"

export class ReactivateDeviceUseCase {
  constructor(private readonly connectionManager: OltConnectionPort) {}

  async execute(dto: OltRequestDTO): Promise<ReactivateDeviceResponseDTO> {
    ...
    try {
      ...
      const result = await adapter.reactivateOnt(pon, serial)
      return result
    } finally {
      await session.close()
    }
