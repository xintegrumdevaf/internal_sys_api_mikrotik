...
import type { DeviceStatusResponseDTO } from "../dto/device-status.response.dto.js"
...
export class GetDeviceStatusUseCase {
  ...
  async execute(dto: OltRequestDTO): Promise<DeviceStatusResponseDTO> {
    ...
    try {
      ...
      const result = await adapter.getDeviceStatus(pon, serial)
      return result
    } finally {
      await session.close()
    }
